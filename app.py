...
asx_scheduled_time = datetime.now().replace(hour=16, minute=0, second=0, microsecond=0)
us_scheduled_time = datetime.now().replace(hour=9, minute=30, second=0, microsecond=0)
...
# Schedule the ASX task at 4:00 PM every day
scheduler.add_job(scheduled_task, 'cron', hour=asx_scheduled_time.hour, minute=asx_scheduled_time.minute, args=[asx_scheduled_time], misfire_grace_time=3600)

# Schedule the US task at 9:00 AM AEST every day
scheduler.add_job(scheduled_task_us, 'cron', hour=us_scheduled_time.hour, minute=us_scheduled_time.minute, timezone='Australia/Sydney', args=[us_scheduled_time], misfire_grace_time=3600)
